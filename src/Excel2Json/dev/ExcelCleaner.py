...
class MDES_CleanUp(object):
    ...
    def __init__(self, sheet_path: str | Path, mongo_client: MongoClient | None = None):
        ...
        if tabs_collection is None:
            raise ValueError(
                "Document 'Tab Names' not found in dictionaries collection."
            )

        self.tabs = tabs_collection["tabs"].values()

        fields_collection = self.dictionaries_collection.find_one(
            {"name": "Field Names by Excel Table Tabs"}
        )
        if fields_collection is None:
            raise ValueError(
                "Document 'Field Names by Excel Table Tabs' not found in dictionaries collection."
            )
    ...
    def execute(self):
        ...
        base["collection"] = (
            base["collection"].replace("No", False).replace("Yes", True)
        )
        base["title_type_main"] = np.repeat("main", len(base))
        base["security_level"] = (
            base["security_level"].copy().apply(self._get_sec_level)
        )
    ...
    def mongo_dictCollection_auth(self) -> MongoClient:
        ...
        warnings.warn(
            "Don't call this method explicitly. Rather, pass an authenticated client to the class constructor.",
            DeprecationWarning,
            stacklevel=2,
        )
